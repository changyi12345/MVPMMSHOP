...
export class AuthService {
  ...
  async register(registerDto: RegisterDto) {
    ...
    const { username, email, password, referralCode, phone, otpCode } = registerDto;
    ...
    if (flags.smsOtpEnabled) {
      if (!phone?.trim() || !otpCode?.trim()) {
        throw new BadRequestException('Phone number and verification code are required');
      }
      const verified = await this.phoneOtp.verifyOtpForRegister(phone, otpCode);
      normalizedPhone = verified.phone;
      phoneVerified = true;
    } else if (phone?.trim()) {
    ...
    const user = await this.prisma.user.create({
      data: {
        ...
        referralCode: code,
        referredById,
        ...(normalizedPhone ? { phone: normalizedPhone, phoneVerified } : {}),
      },
    });
