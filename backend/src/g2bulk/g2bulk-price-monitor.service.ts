...
export class G2bulkPriceMonitorService {
  ...
  async checkPrices(force = false): Promise<{ checked: number; newAlerts: number; pricesUpdated: number }> {
    const now = Date.now();
    if (!force && now - this.lastCheckAt < this.cooldownMs) {
      return { checked: 0, newAlerts: 0, pricesUpdated: 0 };
    }

    try {
      await this.g2bulk.getApiKey();
    } catch {
      return { checked: 0, newAlerts: 0, pricesUpdated: 0 };
    }
    ...
    try {
      ...
      this.logger.warn(`Price check failed: ${err instanceof Error ? err.message : 'error'}`);
      return { checked: 0, newAlerts: 0, pricesUpdated: 0 };
    }
    ...
    return { checked: items.length, newAlerts, pricesUpdated };
  }
  ...
  private async syncProductPrices(
    ...
    for (const item of items) {
      if (item.itemType !== 'voucher') continue;
