...
export class NotificationsService {
  ...
  async sendEmailDetailed(
    ...
    const shopName = shop.shopName?.trim() || 'MVPMMSHOP';
  ...
  async notifyOrderCompleted(
    ...
    await this.sendEmail(
      ...
      `Your order #${orderId} has been completed. Thank you for shopping at MVPMMSHOP!`,
    );
    if (userId) {
    ...
    const flags = await this.settingsService.getFeatureFlags();
    if (flags.smsOrderAlertsEnabled && phone) {
      await this.smsService.sendSms(
        this.smsService.normalizePhone(phone),
        `MVPMMSHOP: Order #${orderId} completed. Thank you!`,
      );
    }
  }
