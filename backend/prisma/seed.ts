...
async function main() {
  ...
  await prisma.user.upsert({
    ...
    create: {
      username: 'admin',
      email: 'admin@mvpmms.com',
      password: adminPassword,
      role: 'ADMIN',
      referralCode: 'MVPMM-ADMIN',
    },
  });
  ...
  if (eventCount === 0) {
    await prisma.shopEvent.create({
      data: {
        ...
        content: 'MVPMMSHOP အွန်လိုင်း shop ဖွင့်လှစ်ခြင်း အခမ်းအနားအတွက် WELCOME10 promo code ကို checkout မှာ သုံးပြီး 10% discount ရယူနိုင်ပါတယ်။\n\nMobile Legends, PUBG, Free Fire top-up နှင့် voucher များ အားလုံး အသုံးပြုနိုင်ပါသည်။',
        isPublished: true,
        isPinned: true,
      },
    });
