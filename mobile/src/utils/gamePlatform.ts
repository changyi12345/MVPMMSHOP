import { isMlbbVariant, MLBB_UNIFIED_CODE } from '../data/mlbb-regions';

export type GamePlatform = 'mobile' | 'pc';
export type GamePlatformFilter = 'all' | GamePlatform;

const PC_CODE_HINTS = [
  'lol', 'valorant', 'dota', 'csgo', 'cs2', 'counter_strike', 'overwatch',
  'diablo', 'wow', 'warcraft', 'starcraft', 'hearthstone', 'league', 'riot',
  'path_of_exile', 'ffxiv', 'lost_ark', 'guild_wars', 'battlenet',
];

const PC_NAME_HINTS = [
  'league of legends', 'valorant', 'dota 2', 'counter-strike', 'overwatch',
  'world of warcraft', 'diablo', 'hearthstone', 'lost ark', 'path of exile',
  'final fantasy xiv', '(pc)', 'pc game',
];

const MOBILE_CODE_HINTS = [
  'mlbb', 'pubgm', 'pubg_mobile', 'freefire', 'free_fire', 'genshin', 'honkai',
  'hsr', 'codm', 'cod_mobile', 'wildrift', 'wild_rift', 'mobile', 'aov',
  'arenaofvalor', 'brawl', 'clash', 'bloodstrike', 'farlight', 'efootball',
  'fcmobile', 'wuthering', 'zenless',
];

const MOBILE_NAME_HINTS = [
  'mobile legends', 'pubg mobile', 'free fire', 'call of duty mobile',
  'wild rift', 'arena of valor', 'mobile game', 'mobile', 'bang bang',
];
...
export function inferGamePlatform(game: { code: string; name: string }): GamePlatform {
  ...
  if (code === MLBB_UNIFIED_CODE || isMlbbVariant(code)) return 'mobile';
